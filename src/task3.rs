...
impl Individual2D {
    ...
    /**
     * Funcion de fitness para Task 3.
     * Es una funcion privada.
     */
    fn update_fitness(&mut self) {
        self.fitness = 5.0 * self.x - self.x.powf(2.0) + 8.0 * self.y - 2.0 * self.y.powf(2.0);
    }
    ...
    fn mutate(&mut self) {
        ...
        while 3.0 * self.x + 2.0 * self.y >= 6.0 {
            self.x = self.x / 2.0;
            self.y = self.y / 2.0;
        }
    }
